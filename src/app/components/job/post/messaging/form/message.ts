...
export class MessageForm implements OnInit, OnChanges, OnDestroy {
  ...
  @Input() jobResponse: JobPostReplyModel;
  ...
  ngOnChanges(changes) {
    ...
    if (changes.jobResponse && changes.jobResponse.currentValue) {
      this.updateFormValues();
    }
  ...
  initForm() {
    this.form = this.builder.group({
      ...
      jobResponse: [this.jobResponse && this.jobResponse['@id'] || '', Validators.required],
  ...
  updateFormValues() {
    if (this.form) {
      ...
      (<Control> this.form.controls['jobResponse'])
        .updateValue(this.jobResponse['@id'], this.controlUpdateOptions);
