...
@Component({
  selector: 'feed-institution-details',
  directives: [
    ...ROUTER_DIRECTIVES,
    NgClass,
    FeedFilter,
...
export class FeedInstitutionDetails implements OnInit, OnChanges {
  ...
  ngOnChanges(changes: any) {
    if (changes.feed && changes.feed.currentValue) {
       let routerLinkArray = [];
      let newName = this.feed.name.trim();
      routerLinkArray[0] = this.feed.routerLink[0];
      routerLinkArray[1] = { id: newName.replace(/[^A-Za-z0-9]+/g,'-')+'?uId='+ this.feed.routerLink[1].id, type: this.feed.routerLink[1].type };
