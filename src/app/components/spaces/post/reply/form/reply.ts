...
import {SpacePostReplyService} from '../services/spacepostreply-service';
...
@Component({
  ...
  providers: [SpacePostReplyService],
  pipes: [JsonPipe, FileSizePipe],
  styles: [ require('./reply.css') ],
  template: require('./reply.html')
})
@CanActivate(checkIfHasPermission)
export class SpacePostReplyForm implements OnInit, OnChanges, OnDestroy {
  @Input() reply: SpacePostReplyModel;
  ...
  constructor(
      ...
      public spacepostReplySrv: SpacePostReplyService,
  ...
  initForm() {
    this.form = this.builder.group({
      ...
      spacePosting: [this.spacepost && this.spacepost['@id'] || '', Validators.required],
  ...
  updateFormValues() {
    if (this.form) {
      ...
      (<Control> this.form.controls['spacePosting'])
        .updateValue(this.spacepost['@id'], this.controlUpdateOptions);
  ...
  saveOrUpdate(formValues: any, $event) {
    ...
    if (this.form.valid) {
      ...
      this.spacepostReplySrv[method](formValues).subscribe(
        ((resp) => {
        ...
        ((resp) => {
          this.notificationsSvc.push(new NotificationModel({
            message: `Error has occured while sending the reply!`,
            type: 'danger'
          }));
        }).bind(this)
      );
